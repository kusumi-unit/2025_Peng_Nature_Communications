...
DobsG = 0.8; % observed total spots density (copies / um^2)
DobsR = 0.8;
Kd3 = 3; % Kd for hetero-dimer (copies / um^2), not availabel for o and inf [0 for stable dimer, inf for random control]

%{
kon1 = 0.47;  % homo-KOR, MOR, DOR: 1.03, 0.53, 0.47; after stim; 1.08, 0.26, 0.43 
koff1 = 8.00; % homo-KOR, MOR, DOR: 6.71, 8.47, 8.00; after stim; 17.86, 5.35, 3.62
kon2 = 0.47;  % homo-dimer2
koff2 = 8.00; % homo-dimer2
koff3 = 3.86;    % hetero-dimer, MOR-DOR:3.86, KOR-DOR: 4.17
%}
Comb = 'MD'; % MD or KD
if strcmp(Comb,'MD')
    kon1 = 0.53;
...
if Homo == 1
    ...
    if Kd3 ~= inf
        [Dtrue_GALL,Dtrue_RALL,Dtrue_GR,Dtrue_GG,Dtrue_RR,Dapp_GR,Dtrue_G,Dtrue_R,Dapp_G,Dapp_R]...
...
Ntrue_GALL = Area*Dtrue_GALL;
Ntrue_RALL = Area*Dtrue_RALL;
Ntrue_GR = Area*Dtrue_GR;
Ntrue_GG = Area*Dtrue_GG;
Ntrue_RR = Area*Dtrue_RR;
...
Gpos = TrackGen(Nframe, Ntrue_GALL, ImSize);
Rpos = TrackGen(Nframe, Ntrue_RALL, ImSize);
...
% Homo-dimerization 
Gpos = HomoGen(Gpos,Ntrue_GG,Nframe);
Rpos = HomoGen(Rpos,Ntrue_RR,Nframe);

% Delete unlabelled spots rondomly
Gpos = DelUnlab(Gpos,EG,Nframe);
Rpos = DelUnlab(Rpos,ER,Nframe);

% Delete incidental dimer and homodimer
if Inci == 1
    for k=1:Nframe
        % Skip if < 1 spots
...
if IndLoop1 == 1 && IndLoop2 == 1
    ...
    Detect_IncCol_hetero(Gd,Gm,Rd,Rm,PixSize,InciTh,DobsG,DobsR,Kd3,Napp_G,Napp_R,Napp_GR,LEr,FilenameRoot)
end
