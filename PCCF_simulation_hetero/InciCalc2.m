function [SumPos, CPos, MPos, Nt_ori, Nt_mod, Ncluser, Nmono] = InciCalc2(Pos, PixSize, InciTh, FigDraw)
...
L=[]; L2=[]; Ncluser=1; Nmono=1; CPos=[]; MPos=[]; CPosOri{1}=[]; D=1;
for cldx=1:Nspots
    L = T == cldx;
    L2 = find(L);
    ...
    if length(L2) >= 2
        ...
        if sum(P{cldx}(:,3)) == 0
            ...
            % Choose biggest dimer ID
            S = sort(P{cldx}(:,3));
            CPos(Ncluser,3) = S(end);
        end
        Ncluser = Ncluser+1;
    else
        MPos(Nmono,1) = P{cldx}(:,1);
        MPos(Nmono,2) = P{cldx}(:,2);
        MPos(Nmono,3) = P{cldx}(:,3);
        Nmono = Nmono+1;
    end
end
% Re-calc #
